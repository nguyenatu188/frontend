...
import RightSidebar from '../components/RightSidebar'; // Adjust path as needed
import useDailyMissions from '../hooks/useDailyMissions'; // Adjust path as needed
...
const Missions = () => {
  ...
  // Update missions from API data
  useEffect(() => {
    console.log('API response:', data);
    if (data?.missions) {
      console.log('Missions from API:', data.missions);
      const sortedMissions = [...data.missions].sort((a, b) => {
        return a.user_mission_id - b.user_mission_id; // Sort ascending by user_mission_id
      });
  ...
  // Handle mission click to show floatbox
  ...
  const handleClaimReward = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/v1/daily-missions/${selectedMission.mission_id}/claim`, {
        method: 'POST',
        headers: {
  ...
  // Get mission icon based on required_action
